...
class Organization extends CI_Controller {
    ...
    public function Division(){
        if($this->session->userdata('user_login_access') != False) { 
        $data['division'] = $this->organization_model->divselect();
        $this->load->view('backend/division',$data);
        }
    else{
		redirect(base_url() , 'refresh');
	}  
    ...
    public function Save_div(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array();
            $data = array('div_name' => $div);
            $success = $this->organization_model->Add_Division($data);
            echo "Successfully Added";
        }
        }
    else{
    ...
    public function div_delete($div_id){
        if($this->session->userdata('user_login_access') != False) {
        $this->organization_model->division_delete($div_id);
        $this->session->set_flashdata('delsuccess', 'Successfully Deleted');
        redirect('organization/Division');
        }
    else{
		redirect(base_url() , 'refresh');
	}        
    }
    public function Edit_div($div){
        if($this->session->userdata('user_login_access') != False) {
        $data['division'] = $this->organization_model->divselect();
        $data['editdivision']=$this->organization_model->division_edit($div);
        $this->load->view('backend/division', $data);
        }
    else{
		redirect(base_url() , 'refresh');
    ...
    public function Update_div(){
        if($this->session->userdata('user_login_access') != False) {
        $id = $this->input->post('id');
        $division = $this->input->post('division');
        $data =  array('div_name' => $division );
        $this->organization_model->Update_Division($id, $data);
        echo "Successfully Updated";
        }
    else{
		redirect(base_url() , 'refresh');
